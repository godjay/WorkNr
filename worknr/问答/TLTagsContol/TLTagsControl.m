...
- (void)reloadTagSubviewsWithTags:(NSMutableArray *)tags{
    ...
    for (int i = 0; i < _tags.count; i++) {
        NSString *str = _tags[i];
        if ([_tags[i] isEqualToString:@"请添加标签"]) {
            tag1 = (NSMutableString *)@"";
        }else if(i < (_tags.count-1)){
            strA = [str stringByAppendingString:@","];
//            NSLog(@"strA  %@",strA);
            tag1 = (NSMutableString *)[tag1 stringByAppendingString:strA];
        }else{
            strA = [str stringByAppendingString:@""];
            tag1 = (NSMutableString *)[tag1 stringByAppendingString:strA];
//            NSLog(@"tag1  %@",tag1);
