...
@interface WTUserInfoController ()
...
@implementation WTUserInfoController
...
- (void)requestData{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@users/%@",apiBaseURL,self.model.rid];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        if ([vc isKindOfClass:[FansCreditController class]]) {
            ...
            comvc.fan.ID = [self.model.rid intValue];
            vc.hidesBottomBarWhenPushed = YES;
